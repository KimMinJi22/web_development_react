...
/**
 * 이벤트 핸들러(onClick) 내에서 함수를 직접 호출했을 경우 컴포넌트가 렌더링
 * 될 때 함수가 호출되어 무한 루프가 발생할 수 있음.
 * 
 * setCount의 경우
 ...
 * 
 * useState를 사용한느 순간에 배열의 0번지는 상태 그 자체에 해당하는
 * 필드명으로 선언되고,
 * 1번지는 setter 개념으로서 0번지의 상태값을 업데이트 하도록 사전에 설정
 * 되어있습니다.
 * 
 * 그렇기 때문에 저희가 따로 setter 형태로 함수를 정의하지 않더라도
 * call2() 유형의(매개변수가 있고 리턴값이 없는) 함수로 만들어지는 것을
 * 의미합니다.
 */
...
/**
 * 이상까지 작성했을 때 App.jsx의 button과 비교를 해보면 이제 하나만 차이가 
 * 납니다. 저희가 작성한 방식은 call3() 유형으로 매개변수는 없는데, return은
 * 있는 형태였습니다.
 * 
 * 그런데 App.jsx를 확인해봤을 때 setCount((count) => count+1)
 * 으로 작성되어 call4()유형(매개변수o / return o)으로 작성되었음을 확인할
 * 수 있습니다.
 * 
 * 
...
// count가 +3씩 이루어져야 할 것 같은데, 결과값을 봤을 경우 +1 씩
// 된다는 점을 확인할 수 있습니다.

/**
 * 이상의 현상이 일어나는 이유 :
 * 1. setCount(count+1)은 현재 상태의 고정된 값을 사용해 업데이트가
 * 일어남
 * 
 * 2. 이상을 이유로 count의 값은 handleClick을 호출했을 시점에
 * '고정' 되어있음.
 * 
 * 3. 1, 2를 이유로 세 번의 setCount는 초기값인 count=0을 시점으로 1을
 * 더해주기 때문에
 * 
 * 0 + 1
 * 0 + 1
 * 0 + 1
 * 로, count = 1 을 return하게 됨.
 * 
